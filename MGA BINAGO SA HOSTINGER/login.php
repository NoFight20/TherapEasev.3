...
if ($_POST && empty($_POST['fp_action']) && ($_POST['action'] ?? '') !== 'resend_verification') {
  ...
  do {
    ...
    $email = norm_email((string)($_POST['useremail'] ?? ''));
    $password = (string)($_POST['userpassword'] ?? '');
    $timestamp = date('H:i:s');
    $datestamp = date('Y-m-d');

    if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL) || $password === '') {
        $error = '<div style="color:#ff3e3e;text-align:center;">Please enter a valid email and password.</div>';
        audit('login_failed', ['reason'=>'invalid_input'], $email);
        break;
    }

    // Rate limit per IP+email
    $key = rl_key($userIP, $email);
    if (rl_is_locked($key, $RATE_LIMIT_MAX_ATTEMPTS, $RATE_LIMIT_WINDOW_SEC, $LOCKOUT_DURATION_SEC)) {
        ...
        audit('login_throttled', ['rl_key' => $key], $email);
        break;
    }
    ...
    <div class="login-card">
      <form action="" method="POST" autocomplete="off">
        ...
        <table>
          ...
          <tr>
            <td class="label-td">
              <input id="useremail" type="email" name="useremail" class="input-text" placeholder="Email Address" required autocomplete="username" inputmode="email">
            </td>
          </tr>
