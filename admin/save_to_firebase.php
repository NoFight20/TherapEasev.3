...
// Retrieve logged in admin's email, specialty and specialty id from session and Firebase
$adminSpecialty = '';
$adminSpecialtyId = '';
    ...
    try {
        ...
        if ($adminData) {
            foreach ($adminData as $key => $value) {
                // Retrieve admin specialty (sname)
                if (isset($value['sname'])) {
                    if (is_array($value['sname'])) {
                        $adminSpecialty = implode(", ", $value['sname']);
                    } else {
                        $adminSpecialty = $value['sname'];
                    }
                }
                // Retrieve admin specialty id 
                if (isset($value['specialty'])) {
                    $adminSpecialtyId = $value['specialty'];
                }
            }
        }
...
if (isset($data['extractedText']) && !empty($data['extractedText'])) {
    ...
    // Add admin specialty and specialty id to the parsed data
    $parsedData['sname'] = $adminSpecialty;
    $parsedData['specialty'] = $adminSpecialtyId;
    
    if (!empty($parsedData)) {
        // Generate a unique key to store in both nodes
        $uniqueKey = $database->getReference('ocr_results')->push()->getKey();
        
        // Store the extracted data under the generated key in both nodes
    ...
    if (file_exists($uploadedFile)) {
        try {
            ...
            file_put_contents('debug_log.txt', "IOFactory Error: " . $e->getMessage() . "\n", FILE_APPEND);
            echo json_encode(['success' => false, 'error' => 'Failed to load DOCX file: ' . $e->getMessage()]);
            exit;
        }
        $text = '';
