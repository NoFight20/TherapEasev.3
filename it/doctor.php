...
if (isset($_POST['fname'], $_POST['lname'], $_POST['doctorEmail'], $_POST['password'], $_POST['confirm_password'], $_POST['specialty'])) {
    $fname              = $_POST['fname'];
    $lname              = $_POST['lname'];
    $doctorEmail        = $_POST['doctorEmail'];
    $docPassword        = $_POST['password'];
    $docConfirmPassword = $_POST['confirm_password'];
    ...
    // For doctors, set user type to 'd' and node path to 'doctor'
    $userType = 'd';
    $nodePath = 'doctor';

    // Hash the doctor password before storing it in the database
    $hashedPassword = password_hash($docPassword, PASSWORD_DEFAULT);
    ...
    try {
        ...
        // Save user data in Firebase Realtime Database under the "doctor" node
        ...
        echo "Error creating doctor account: " . $e->getMessage();
    }
}
...
<body>
    ...
    <div class="container">
                ...
                <tr class="menu-row">
                    ...
                    <tr class="menu-row">
                        <td class="menu-btn menu-active">
                            <a href="doctor.php" class="non-style-link-menu non-style-link-menu-active">
		...
		<div class="dash-body">
			...
			<tr >
            <td width="13%" >
					<a href="doctor.php" ><button  class="login-btn btn-primary-soft btn btn-icon-back"  style="padding-top:11px;padding-bottom:11px;margin-left:20px;width:125px"><font class="tn-in-text">Back</font></button></a>
                </td>
				<td>
                <form action="doctor.php" method="POST" class="header-search">
                    <input type="search" name="search" class="input-text header-searchbar" placeholder="Search Doctor name or Email" list="doctor">&nbsp;&nbsp;
            ...
            <?php
try {
    // Reference to the 'doctor' node in Firebase
    $doctorRef = $database->getReference('doctor');
    $doctors = $doctorRef->getValue();

    // Count the number of doctors
    $doctorCount = $doctors ? count($doctors) : 0;
} catch (Exception $e) {
    echo "Error retrieving doctor data: " . $e->getMessage();
    $doctorCount = 0;
}
?>
                            ...
                            <tr >
                                <td colspan="2" style="padding-top:30px;">
                                    <p class="heading-main12" style="margin-left: 45px;font-size:20px;color:rgb(49, 49, 49)">Doctor (<?php echo  $doctorCount; ?>)</p>
                                </td>
                                <td colspan="2">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle search query for doctors
        ...
        try {
            ...
            if ($doctors) {
                foreach ($doctors as $key => $doctor) {
                    if (
                        stripos($doctor['name'], $searchTerm) !== false || 
                        stripos($doctor['email'], $searchTerm) !== false
                    ) {
                        $searchResults[$key] = $doctor;
                    }
                }
            }
    ...
    elseif (isset($_POST['update_password'])) {
        ...
        try {
            // Update password in Firebase Realtime Database for the doctor
            $database->getReference("doctor/$doctorKey")->update([
                'password' => $hashedPassword,
            ]);

            // Retrieve the doctor data to log details
            $doctorData = $database->getReference("doctor/$doctorKey")->getValue();
            $doctorEmail = $doctorData['email'] ?? 'unknown';
            $specialtyKey = isset($doctorData['specialty']) ? $doctorData['specialty'] : '';
    ...
    elseif (isset($_POST['remove_doctor'])) {
        ...
        try {
            // Retrieve doctor data before removal to log details
            $doctorData = $database->getReference("doctor/$removeKey")->getValue();
            $doctorEmail = $doctorData['email'] ?? 'unknown';
            $specialtyKey = isset($doctorData['specialty']) ? $doctorData['specialty'] : '';
            $specialtyName = ($specialtyKey && isset($specialtiesList[$specialtyKey])) ? $specialtiesList[$specialtyKey] : '';

            // Remove doctor from Firebase
            $database->getReference("doctor/$removeKey")->remove();
            $database->getReference("users/$removeKey")->remove();

            // Log the doctor removal action
            $logEntry = [
                'status'         => 'Doctor Removed',
                'created_by'     => $_SESSION['user'] ?? 'unknown',
                'created_for'    => $doctorEmail,
                'timestamp'      => $timestamp,
                'datestamp'      => $datestamp,
            ...
            echo "<script>alert('Doctor account removed successfully!');</script>";

            // Refetch data after removal
            $doctorRef = $database->getReference('doctor');
            $doctors = $doctorRef->getValue();
            $displayAccounts = $doctors ? $doctors : [];
        } catch (Exception $e) {
            echo "Error removing doctor: " . $e->getMessage();
            $displayAccounts = [];
        }
    }
} else {
    // Default: Display all doctor accounts
    try {
        $doctorRef = $database->getReference('doctor');
        $doctors = $doctorRef->getValue();
        $displayAccounts = $doctors ? $doctors : [];
...
if (!empty($displayAccounts)) {
    // Sort $displayAccounts first by specialty (alphabetically) then by doctor's name (alphabetically)
    uasort($displayAccounts, function($a, $b) {
        $cmp = strcasecmp($a['sname'] ?? '', $b['sname'] ?? '');
        if ($cmp === 0) {
    ...
    echo '<thead>
            <tr>
                ...
                <th class="table-headin">Remove Doctor</th>
            </tr>
          </thead>';

    foreach ($displayAccounts as $key => $doctor) {
        // Instead of using specialtiesList, get the specialty name directly from "sname"
        $specialtyName = isset($doctor['sname']) ? htmlspecialchars($doctor['sname']) : 'N/A';

        echo "<tr style='text-align: center; vertical-align: middle;'>
                <td style='padding: 10px 0; border-bottom: 1px solid #ddd;'>" . htmlspecialchars($doctor['name']) . "</td>
                <td style='padding: 10px 0; border-bottom: 1px solid #ddd;'>" . htmlspecialchars($doctor['email']) . "</td>
                <td style='padding: 10px 0; border-bottom: 1px solid #ddd;'>" . $specialtyName . "</td>
                <td style='padding: 10px 0; border-bottom: 1px solid #ddd;'>
                    <!-- Existing Edit button for password change -->
                    <button onclick=\"showEditPopup('$key', '" . htmlspecialchars($doctor['name']) . "')\"
                ...
                <td style='padding: 10px 0; border-bottom: 1px solid #ddd;'>
                    <!-- New Remove button -->
                    <form method='POST' onsubmit=\"return confirm('Are you sure you want to remove this doctor?');\" style='display:inline;'>
                        <input type='hidden' name='remove_key' value='$key'>
                        <button type='submit' name='remove_doctor' class='login-btn btn-danger btn'>
