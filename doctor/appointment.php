        ...
        <div class="dash-body">
            <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
                <tr >
                    ...
                    <td width="15%">
                        ...
                        <p class="heading-sub12" style="padding: 0;margin: 0;">
                            ...
                            $schedules = $scheduleRef->orderByChild('name')->equalTo($userid)->getValue();

                            if ($schedules) {
                                foreach ($schedules as $schedule) {
                                    if (isset($schedule['scheduledate']) && $schedule['scheduledate'] === $today) {
                                        ...
                                        foreach ($appointments as $appointment) {
                                            ...
                                            // Fetch patient details using PID
                                            $patientRef = $database->getReference('patient/' . $pid)->getValue();
                                            $patientName = $patientRef['name'] ?? 'Unknown';

                                            echo "Schedule ID: " . $scheduleid . "<br>";
                                            echo "Doctor ID: " . $docid . "<br>";
                                            echo "Patient ID: " . $pid . "<br>";
                                            echo "Patient Name: " . $patientName . "<br>";
                                            echo "Appointment Date: " . $schedule['scheduledate'] . "<br><br>";
                                        }
                                    }
                                }
                            } else {
                ...
                <?php
                    ...
                    // Check if there’s a specific date from the form submission
                    $sheduledate = $_POST["sheduledate"] ?? null;

                    // Fetch schedules based on doctor ID and optional schedule date
                    $schedulesQuery = $scheduleRef->orderByChild('name')->equalTo($userid);
                    $schedules = $schedulesQuery->getValue();

                    if ($schedules) {
                        foreach ($schedules as $schedule) {
                            // Filter by schedule date if it's set
                            if ($sheduledate && isset($schedule['scheduledate']) && $schedule['scheduledate'] !== $sheduledate) {
                                continue;
                            }
                            ...
                            // Fetch appointments for the current schedule ID
                            ...
                            if ($appointments) {
                                foreach ($appointments as $appointment) {
                                    ...
                                    $patient = $database->getReference('patient/' . $pid)->getValue();
                                    $pname = $patient['pname'] ?? 'Unknown';

                                    // Fetch doctor details
                                    $doctor = $database->getReference('doctor/' . $userid)->getValue();
                                    $docname = $doctor['docname'] ?? 'Unknown';
                                    ...
                                    echo "Schedule Date: " . $scheduledate . "<br>";
                                    echo "Schedule Time: " . $scheduletime . "<br>";
                                    echo "Appointment Number: " . $apponum . "<br>";
                                    echo "Appointment Date: " . $appodate . "<br><br>";
                                }
                            }
                        }
                    } else {
